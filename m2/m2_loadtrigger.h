...
M2_API int m2_loadtrigger_on_phase(int target_idx, m2_phase_cb cb, void* ud);

/* Begin tracking: subscribes to the shared log hook and installs it. Idempotent. */
M2_API int m2_loadtrigger_install(void);

/* Highest phase index reached so far (-1 before any). */
M2_API int m2_loadtrigger_current_phase(void);

/* Human-readable phase name for an index, or "?" if out of range. */
M2_API const char* m2_loadtrigger_phase_name(int idx);
